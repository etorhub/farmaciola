...
from custom_components.farmaciola.const import (
    CONF_NOTIFY_SERVICE,
...
async def test_user_step_creates_entry_with_input():
    ...
    await flow.async_step_user({CONF_NOTIFY_SERVICE: "notify.mobile"})
...
async def test_options_flow_init_step_updates_entry_with_input():
    ...
    await flow.async_step_init({CONF_NOTIFY_SERVICE: "notify.new"})
