...
.title { font-size: 1.350rem; font-weight: 700; flex-shrink: 0; }
.search {
  flex: 1; min-width: 140px;
  background: var(--input-fill-color, var(--secondary-background-color));
  border: 1.5px solid var(--divider-color); border-radius: 999px;
  padding: 7px 14px; font-size: 1.125rem; color: var(--primary-text-color); outline: none;
}
...
.btn-secondary {
  ...
  border: 1.5px solid var(--divider-color); border-radius: 999px;
  padding: 7px 14px; font-size: 1.125rem; font-weight: 600; cursor: pointer;
}
.btn-danger {
  background: var(--error-color, #c62828); color: white; border: none;
  border-radius: 999px; padding: 7px 14px; font-size: 1.125rem; font-weight: 600; cursor: pointer;
}
.med-row {
  display: flex; align-items: center; gap: 12px;
...
.empty { text-align: center; color: var(--secondary-text-color); padding: 32px 16px; }
...
.stat-card {
  flex: 1; background: var(--secondary-background-color); border-radius: 12px;
...
class FarmaciolaCard extends HTMLElement {
  constructor() {
    super();
  ...
  _renderList() {
    ...
    if (!meds.length) {
      ...
      list.querySelector("#addLink")?.addEventListener("click", () => this._openAdd());
      return;
    }
  ...
  _renderForm(editing) {
    ...
    ov.querySelectorAll(".ac-item").forEach((item) =>
      item.addEventListener("click", () => this._selectCima(item.dataset.nr, editing))
    );
    ...
    if (cimaInput) {
      ...
      cimaInput.addEventListener("input", (e) => {
        ...
        this._cimaDebounce = setTimeout(() => this._searchCima(editing), 300);
      });
    }
    ...
    ov.querySelector("#saveBtn").addEventListener("click", () => this._save(editing));
  }

  async _searchCima(editing) {
    try {
      this._cimaResults = await this._api("GET", `cima/search?q=${encodeURIComponent(this._cimaQuery)}`);
    } catch {
      this._cimaResults = [];
    }
  ...
  async _selectCima(nregistro, editing) {
    try {
      const detail = await this._api("GET", `cima/medicamento?nregistro=${nregistro}`);
  ...
  async _save(editing) {
    ...
    if (!noCaduca && !expiryVal) { alert("Indica la fecha de caducidad o marca 'Sin fecha de caducidad'"); return; }
    const fecha_caducidad = noCaduca ? null : expiryVal + "-01";
    const notas = ov.querySelector("#notes")?.value || "";
    let body;
    if (this._formMode === "cima") {
      if (!this._form.nregistro && !editing) { alert("Please select a medicine from CIMA"); return; }
      body = { ...this._form, fecha_caducidad, no_caduca: noCaduca, notas };
    } else {
      const nombre = ov.querySelector("#mName")?.value?.trim();
      if (!nombre) { alert("Name is required"); return; }
      body = {
        source: "manual",
        nombre,
...
customElements.define("farmaciola-card", FarmaciolaCard);
